...
- Pedro's fork also offers an opt-in **Personal Cloud Farm** backed by a private Google Cloud Firestore document. A high-entropy pairing link binds only the active Local Farm profile on each device; the browser save remains primary, transactional writes use an exclusive 10-minute device lease, and a second device must explicitly take over. The private key stays in the URL fragment/local storage and is never embedded in the public bundle. On iOS, Safari creates a short-lived, one-time install ticket so the Home Screen app can pair its otherwise isolated storage without putting the private key in a cookie.
